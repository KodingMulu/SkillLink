...
export default function UserManagementPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await axios.get<UserListResponse>(
        getApiUrl('/user/admin/list-user'),
        {
          params: {
  ...
  const getColorStyles = (colorName: string) => {
    switch (colorName) {
      case 'blue': return { bg: 'bg-blue-50 text-blue-600 border-blue-100' };
      case 'orange': return { bg: 'bg-orange-50 text-orange-600 border-orange-100' };
      case 'emerald': return { bg: 'bg-emerald-50 text-emerald-600 border-emerald-100' };
      default: return { bg: 'bg-slate-50 text-slate-600 border-slate-100' };
    }
  };
    ...
    <DashboardLayout role="admin">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {loadingStats ? (
            [...Array(3)].map((_, i) => (
              <div key={i} className="bg-white p-4 rounded-2xl border border-slate-200 h-24 animate-pulse" />
            ))
          ) : (
            (stats || []).map((stat, idx) => {
              ...
              return (
                <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 flex items-center gap-4 shadow-sm hover:border-slate-300 transition-all">
                  <div className={`w-12 h-12 ${styles.bg} border rounded-xl flex items-center justify-center font-bold`}>
                ...
                {loadingUsers ? (
                  ...
                  (users || []).map((user) => (
                    <tr key={user.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${user.role === 'FREELANCER' ? 'text-purple-700 bg-purple-50 border border-purple-100' : 'text-blue-700 bg-blue-50 border border-blue-100'}`}>
                          {user.role}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-600">
                        {user.joined ? new Date(user.joined).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {user.status === 'pending' && (
                            <button title="Verifikasi User" className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition border border-slate-200 bg-white shadow-sm">
                              <UserCheck className="w-4 h-4" />
                            </button>
                          )}
                          <button title="Edit User" className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition border border-slate-200 bg-white shadow-sm">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          <button title="Suspend User" className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition border border-slate-200 bg-white shadow-sm">
